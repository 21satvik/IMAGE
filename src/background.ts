...
var renderingsPanel: browser.Tabs.Tab;
      ...
      case "full":
        {
          ...
          var progressWindow = await browser.tabs.create({
          ...
          try {
            ...
            browser.tabs.remove(progressWindow.id!)
            if (resp.ok) {
          ...
          if (json["renderings"].length > 0) {
            if (query["request_uuid"] !== undefined) {
              ...
              if (renderingsPanel !== undefined) {
                try {
                  await browser.tabs.remove(renderingsPanel.id!)
